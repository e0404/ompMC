...
/******************************************************************************
 ...
 The weights modulate FLUENCE, not spectrum: a weight of 0.02 for a leaf that
 transmits 2% starts 2% of the particles it would otherwise have started, with
 the unhardened spectrum. Attenuation through the collimator, its scatter and
 the beam hardening that comes with it are not modelled.
...
struct OmcForwardOptions {
    ...
    /* 1 : dose in Gy for the weights given, 0 : mean deposited energy */
    int outputDose;
};

struct OmcForwardCallbacks {
    /* Fraction of the calculation finished, in [0,1]. Optional; called once
     per batch on the master thread, outside any parallel region.

     Return 0 to abandon the calculation. It stops after the current batch and
     returns 0 without touching dose[] or uncertainty[] -- the batches are
     averaged, so a run that stopped halfway would be a dose with no meaning.
     Return nonzero to carry on. */
...
/* Transport the histories and write the results into dose[] and, unless it is
 ...
 0.9999999 wherever nothing was deposited, the same convention omcCalcCube()
 follows.

 Returns nonzero when the run finished, 0 when the progress callback stopped
 it. */
